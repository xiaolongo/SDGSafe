...
class Logger_classify(object):
    """
    logger for node classification task
    reporting train/valid/test accuracy or rocauc for classification
    """
...
class Logger_detect(object):
    ...
    reporting test auroc/aupr/fpr95 for ood detection
    """
    ...
    def print_statistics(self, run=None):
        if run is not None:
            result = 100 * torch.tensor(self.results[run])
            ood_result, test_score, valid_loss = (
                result[:, :-2],
                result[:, -2],
                result[:, -1],
            )
            ...
            for k in range(result.shape[1] // 3):
                ...
                print(
                    f"OOD Test {k + 1} Final AUPR: {ood_result[argmin, k * 3 + 1]:.2f}"
                )
                print(
                    f"OOD Test {k + 1} Final FPR95: {ood_result[argmin, k * 3 + 2]:.2f}"
                )
            ...
            if best_result.shape[0] == 1:
                ...
                for k in range(ood_te_num):
                    r = best_result[:, k * 3]
                    print(
                        f"OOD Test {k + 1} Final AUROC: {r.mean():.2f} ± {r.std():.2f}"
                    )
                    r = best_result[:, k * 3 + 1]
                    print(
                        f"OOD Test {k + 1} Final AUPR: {r.mean():.2f} ± {r.std():.2f}"
                    )
                    r = best_result[:, k * 3 + 2]
    ...
    with open(f"{filename}", "a+") as write_obj:
        ...
        if results.shape[0] == 1:  # one run
            ...
            for k in range(results.shape[1] // 3):
                ...
                r = results[:, k * 3 + 2]
                write_obj.write(
                    f"OOD Test {k + 1} Final FPR: {r.mean():.2f} ± {r.std():.2f}\n"
                )
            r = results[:, -1]
